...
byte* read_duel_script(void* pduel, const char* script_name, int* len) {
	return ((duel*)pduel)->sreader(script_name, len);
}
uint32 read_duel_card(void* pduel, uint32 code, card_data* data) {
	return ((duel*)pduel)->creader(code, data);
}
uint32 handle_duel_message(void* pduel, uint32 msg_type) {
	return ((duel*)pduel)->mhandler(pduel, msg_type);
}
...
extern "C" DECL_DLLEXPORT ptr create_duel_ex(uint32 seed, script_reader sreader_, card_reader creader_, message_handler mhandler_) {
	duel* pduel = new duel(sreader_, creader_, mhandler_);
	duel_set.insert(pduel);
	pduel->random.reset(seed);
	return (ptr)pduel;
}
