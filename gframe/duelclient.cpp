...
bool DuelClient::externalServer = false;

bool DuelClient::is_refreshing = false;
int DuelClient::match_kill = 0;
std::vector<HostPacket_V2> DuelClient::hosts;
...
void DuelClient::ClientRead(bufferevent* bev, void* ctx) {
	...
	while(true) {
		...
		{
			if(!externalServer)
				HandleSTOCPacketLan<STOC_ErrorMsg, STOC_JoinGame, STOC_TimeLimit>(&duel_client_read[2], packet_len);
			else
				HandleSTOCPacketLan<STOC_ErrorMsg_AlignDefault, STOC_JoinGame_AlignDefault, STOC_TimeLimit_AlignDefault>(&duel_client_read[2], packet_len);
		}
		len -= packet_len + 2;
	}
}
		...
		if(!is_closing) {
			if(connect_state == 0x1) {
				...
				if(!mainGame->dInfo.isStarted && !mainGame->is_building) {
					...
					mainGame->globalEventHandler.SetGameEventHandler(&mainGame->menuHandler);
#ifdef _IRR_ANDROID_PLATFORM_
...
template<typename STOC_ErrorMsgStruct, typename STOC_JoinGameStruct, typename STOC_TimeLimitStruct>
void DuelClient::HandleSTOCPacketLan(char* data, unsigned int len) {
	...
	case STOC_ERROR_MSG: {
		STOC_ErrorMsgStruct* pkt = (STOC_ErrorMsgStruct*)pdata;
		switch(pkt->msg) {
	...
	case STOC_CHANGE_SIDE: {
		...
		{
			int* p = (int*)pdata;
			int mainExtraCount = *p; p += 1;
			int sideCount = *p; p += 1;

			deckManager.LoadDeck(deckManager.current_deck, p, mainExtraCount, sideCount);
			mainGame->deckBuilder.pre_mainc = deckManager.current_deck.main.size();
			mainGame->deckBuilder.pre_extrac = deckManager.current_deck.extra.size();
			mainGame->deckBuilder.pre_sidec = deckManager.current_deck.side.size();
		}
		...
		mainGame->gMutex.Unlock();
		mainGame->soundEffectPlayer->doInSideBGM();
		break;
	}
	...
	case STOC_JOIN_GAME: {
		...
		STOC_JoinGameStruct* pkt = (STOC_JoinGameStruct*)pdata;
		std::wstring str;
		wchar_t msgbuf[256];
	...
	case STOC_DUEL_START: {
		...
		mainGame->gMutex.Lock();
		User::Instance()->SetSkinWindowUnTrans();
		mainGame->dField.Clear();
	...
	case STOC_REPLAY: {
		...
		char timebuf[40];
		strftime(timebuf, 40, "%Y-%m-%d %H-%M-%S", localtime(&nowtime));
	...
	case STOC_TIME_LIMIT: {
		STOC_TimeLimitStruct* pkt = (STOC_TimeLimitStruct*)pdata;
	...
	case STOC_HS_PLAYER_ENTER: {
		...
		mainGame->gMutex.Unlock();
#if WIN32
		static std::atomic_bool isFlashing = false;
		if (!isFlashing)
		{
			isFlashing = true;
			FLASHWINFO fwi;
			...
			if (mainGame->gameConf.use_d3d)
				fwi.hwnd = (HWND)mainGame->driver->getExposedVideoData().D3D9.HWnd;
			else
				fwi.hwnd = (HWND)mainGame->driver->getExposedVideoData().OpenGLWin32.HWnd;
			HWND curHwnd = GetForegroundWindow();
			if (fwi.hwnd != curHwnd)
			{
				BOOL result = SetForegroundWindow(fwi.hwnd);
			}
...
int DuelClient::ClientAnalyze(char * msg, unsigned int len) {
	...
	case MSG_WIN: {
		...
		if (player == 2)
		{
			mainGame->showcardcode = 3; //draw game
			mainGame->soundEffectPlayer->doBGMLose(); 
		}
		else if(mainGame->LocalPlayer(player) == 0) {
			mainGame->showcardcode = 1;
			mainGame->soundEffectPlayer->doBGMWin();
			if(match_kill)
	...
	case MSG_NEW_TURN: {
		...
		if(!mainGame->dInfo.isReplay && mainGame->dInfo.player_type < 7) {
			if(!mainGame->chkHideChainButton->isChecked()) {
				...
				mainGame->btnRightClick->setVisible(true);
...
void DuelClient::BeginRefreshHost() {
	...
	HostRequest hReq;
	hReq.identifier = NETWORK_CLIENT_ID;
	User::Instance()->QueryRoomList_V2(hReq, &HandleHostInfoPacket_V2);
#endif
}
...
void DuelClient::BroadcastReply(evutil_socket_t fd, short events, void * arg) {
	if (events & EV_TIMEOUT) {
		...
		if (!is_closing && pHP->identifier == NETWORK_SERVER_ID && pHP->version == PRO_VERSION && remotes.find(ipaddr) == remotes.end()) {
			...
			hoststr.append(L"]");
			wchar_t gamename[ARRAYSIZE(pHP->name)];
			BufferIO::CopyWStr(pHP->name, gamename, ARRAYSIZE(gamename));
...
*/
/*
void DuelClient::HandleHostInfoPacket(const std::vector<HostPacket> hostInfo)
{
	for (auto& HP : hostInfo)
	{
		HostPacket* pHP = (HostPacket*)&HP;
		if (!is_closing && pHP->identifier == NETWORK_SERVER_ID && pHP->version == PRO_VERSION) {
			hosts.push_back(*pHP);
			std::wstring hoststr;
			hoststr.append(L"[");
			hoststr.append(deckManager.GetLFListName(pHP->host.lflist));
...
*/
void DuelClient::HandleHostInfoPacket_V2(const std::vector<HostPacket_V2> hostInfo)
{
	for (auto& HP : hostInfo)
	{
		HostPacket_V2* pHP = (HostPacket_V2*)&HP;
		if (!is_closing && pHP->identifier == NETWORK_SERVER_ID && pHP->version == PRO_VERSION) {
			hosts.push_back(*pHP);
			std::wstring hoststr;
			hoststr.append(L"[");
			...
			hoststr.append(L"]");
			wchar_t gamename[270];
			wcscpy(gamename, pHP->name.ToWString().c_str());
			hoststr.append(gamename);
			mainGame->lstHostList->addItem(hoststr.c_str());
		}
	}
...
bool DuelClient::IsInited()
{
	return client_bev;
}

void DuelClient::AdjustLPBGM(int player, int lp)
{
	if (selftype == player)
	{
		auto mylp = lp;
		...
		if (mylp - yourlp > 4000)
		{
			mainGame->soundEffectPlayer->doMoreLpBGM();
		}
		else if (mylp <= 3500)
		{
			mainGame->soundEffectPlayer->doLowLpBGM();
		}
	}
}
