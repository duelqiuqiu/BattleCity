		...
		if(mainGame->dInfo.curMsg != MSG_SORT_CHAIN && mainGame->dInfo.curMsg != MSG_SORT_CARD) {
			...
			if(selectable_cards[i]->location == LOCATION_OVERLAY) {
				...
				if(selectable_cards[i]->overlayTarget->controler)
					mainGame->stCardPos[i]->setBackgroundColor({ 128,38,38,38 });
				else mainGame->stCardPos[i]->setBackgroundColor({255,0,0,0});
			} else {
			...
			mainGame->stCardPos[i]->setBackgroundColor({ 255,0,0,0 });
		}
...
void ClientField::UpdateDeclarableCode(bool enter) {
	...
	for(auto cit = dataManager._strings.begin(); cit != dataManager._strings.end(); ++cit) {
		if(std::wstring(cit->second.name).find(pname) != std::wstring::npos) {
