...
void Replay::BeginRecord() {
	...
	if(is_recording)
		fclose(fp);
	fp = fopen((const char*)mainGame->fileManager->GetRealPath(L"./replay/_LastReplay.yrp").ToString().c_str(), "wb");
	if(!fp)
		return;
#endif
	pdata = replay_data;
...
void Replay::SaveReplay(const wchar_t* name) {
	wchar_t fname[64];
	myswprintf(fname, mainGame->fileManager->GetRealPath( L"./replay/%ls.yrp").ToWString().c_str(), name);
#ifdef WIN32
