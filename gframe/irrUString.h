...
public:
	...
	ustring16<TAlloc>& append(wchar_t character) {
		if (sizeof(wchar_t) == 4)
			return append((const uchar32_t)character);
		else
			return append(character);
	}

	ustring16<TAlloc>& append(const char character) {
		return append((const uchar8_t)character);
	}
	...
	ustring16<TAlloc>& append(const wchar_t* const other, u32 length = 0xffffffff) {
		if (sizeof(wchar_t) == 4)
			return append((const uchar32_t* const)other, length);
		else
			return append((const uchar16_t* const)other, length);
	}
	...
	ustring16<TAlloc>& append(const char* other) {
		return append((const uchar8_t*)other);
	}
...
*/
	...
	core::stringw toStringW() const
	{
		return core::stringw(this->c_str());
	}
