...
void CGUIImageButton::draw() {
	...
	pos.X -= (s32)(this->RelativeRect.getWidth() * imageScale.X * 0.5f);
	pos.Y -= (s32)(this->RelativeRect.getHeight() * imageScale.Y * 0.5f);
	if(Pressed) {
		pos.X += 1;
		pos.Y += 1;
		center.X += 1;
	...
	float sw = (float)this->AbsoluteRect.getWidth() / (float)ImageRect.getWidth();
	float sy = (float)AbsoluteRect.getHeight() / (float)ImageRect.getHeight();
	if(Image && isDrawImage)
		irr::gui::Draw2DImageRotation(driver, Image, ImageRect, pos, center, imageRotation, {sw * imageScale.X, sy * imageScale.Y });
	IGUIElement::draw();
}
