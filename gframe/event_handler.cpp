...
bool ClientField::OnEvent(const irr::SEvent& event) {
	...
	case irr::EET_GUI_EVENT: {
		...
		case irr::gui::EGET_BUTTON_CLICKED: {
			switch(id) {
			case BUTTON_RIGHTCLICK:
				goto LABEL_MOUSERIGHTCLICK;
			...
			case SCROLL_CARD_SELECT: {
				...
				for(int i = 0; i < 5; ++i) {
					...
					if(selectable_cards[i + pos]->location == LOCATION_OVERLAY) {
						...
						else mainGame->stCardPos[i]->setBackgroundColor({255,38,38,38}); // ¸Ä
					} else {
						if(selectable_cards[i + pos]->is_selected)
							mainGame->stCardPos[i]->setBackgroundColor(0xffffff00);
						else if(selectable_cards[i + pos]->controler)
							mainGame->stCardPos[i]->setBackgroundColor(0xffd0d0d0);
						else mainGame->stCardPos[i]->setBackgroundColor({ 255,38,38,38 });
					}
				}
				break;
			}
			case SCROLL_CARD_DISPLAY: {
				...
				for(int i = 0; i < 5; ++i) {
					...
					if(display_cards[i + pos]->location == LOCATION_OVERLAY) {
						...
						else mainGame->stDisplayPos[i]->setBackgroundColor({ 255,38,38,38 });
					} else {
...
void ClientField::ShowMenu(int flag, int x, int y) {
	...
	irr::core::list<irr::gui::IGUIElement*> children;
	...
	if(flag & COMMAND_ACTIVATE) {
		...
		children.push_back(mainGame->btnActivate);
	...
	if(flag & COMMAND_SUMMON) {
		...
		height += 40 * mainGame->yScale;
		children.push_back(mainGame->btnSummon);
	...
	if(flag & COMMAND_SPSUMMON) {
		...
		height += 40 * mainGame->yScale;
		children.push_back(mainGame->btnSPSummon);
	...
	if(flag & COMMAND_MSET) {
		...
		height += 40 * mainGame->yScale;
		children.push_back(mainGame->btnMSet);
	...
	if(flag & COMMAND_SSET) {
		...
		height += 40 * mainGame->yScale;
		children.push_back(mainGame->btnSSet);
	...
	if(flag & COMMAND_REPOS) {
		...
		height += 40 * mainGame->yScale;
		children.push_back(mainGame->btnRepos);
	...
	if(flag & COMMAND_ATTACK) {
		...
		height += 40 * mainGame->yScale;
		children.push_back(mainGame->btnAttack);
	...
	if(flag & COMMAND_LIST) {
		...
		height += 40 * mainGame->yScale;
		children.push_back(mainGame->btnShowList);
