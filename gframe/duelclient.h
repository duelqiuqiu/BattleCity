...
public:
	...
	static void AdjustLPBGM(int player, int val);
...
public:
	static std::vector<HostPacket_V2> hosts;
	static void BeginRefreshHost();
	static int RefreshThread(void* arg);
	static void BroadcastReply(evutil_socket_t fd, short events, void* arg);
	static void HandleHostInfoPacket(const std::vector<HostPacket> hostInfo);
	static void HandleHostInfoPacket_V2(const std::vector<HostPacket_V2> hostInfo);

	static bool IsInited();

	static bool externalServer;
};
