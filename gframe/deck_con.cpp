	...
	bool DeckBuilder::OnEvent(const irr::SEvent& event) {
		...
		case irr::EET_GUI_EVENT: {
			if(event.GUIEvent.EventType == irr::gui::EGET_CHECKBOX_CHANGED)
				mainGame->soundEffectPlayer->doCheckBoxChange();
			...
			case irr::gui::EGET_BUTTON_CLICKED: {
				...
				case BUTTON_CLEAR_DECK: {
					ButtonClicked_ClearDeck();

					break;
				}
				case BUTTON_SORT_DECK: {
					ButtonClicked_SortDeck();
					break;
				}
				case BUTTON_SHUFFLE_DECK: {
					ButtonClicked_ShuffleDeck();

					break;
				}
				case BUTTON_SAVE_DECK: {
					ButtonClicked_SaveDeck();

					break;
				}
				case BUTTON_SAVE_DECK_AS: {
					ButtonClicked_SaveDeckAs();

					break;
				}
				case BUTTON_DELETE_DECK: {
					ButtonClicked_DeleteDeck();

					break;
				}
				case BUTTON_LEAVE_GAME: {
					ButtonClicked_LeaveGame();
				...
				case BUTTON_CATEGORY_OK: {
					ButtonClicked_CategoryOK();

					break;
				}
				case BUTTON_SIDE_OK: {
					ButtonClicked_SideOK();
			...
			case irr::gui::EGET_COMBO_BOX_CHANGED: {
				...
				case COMBOBOX_DBLFLIST: {
					ComboBoxChanged_DBLFList();

					break;
				}
				case COMBOBOX_DBDECKS: {
					ComboBoxChanged_DBDecks();

					break;
				}
				case COMBOBOX_MAINTYPE: {
					ComboBoxChanged_MainType();

					break;
				}
				case COMBOBOX_SORTTYPE: {
					ComboBoxChanged_SortType();
		...
		for (code_pointer ptr = dataManager._datas.begin(); ptr != dataManager._datas.end(); ++ptr, ++strpointer) {
			...
			case 3: {
				...
				break;
					...
					if (pstr) {
						if (pstr[0] == L'$') {
							if (std::wstring(text.name).find(&pstr[1]) == std::wstring::npos)
								continue;
						}
							...
							if (std::wstring(text.name).find(pstr) == std::wstring::npos && std::wstring(text.text).find(pstr) == std::wstring::npos)
								continue;
						}
					}
					results.push_back(ptr);
			...
			void DeckBuilder::ButtonClicked_ClearDeck()
			{
			...
			void DeckBuilder::ButtonClicked_SortDeck()
			{
			...
			void DeckBuilder::ButtonClicked_SaveDeckAs()
			{
			...
			void DeckBuilder::ButtonClicked_DeleteDeck()
			{
			...
			void DeckBuilder::ButtonClicked_LeaveGame()
			{
				...
				mainGame->soundEffectPlayer->doBack();
				if (exit_on_return)
					mainGame->device->closeDevice();
			}

			void DeckBuilder::ButtonClicked_StartFilter()
			{
			...
			void DeckBuilder::ButtonClicked_CategoryOK()
			{
				filter_effect = 0;
			...
			void DeckBuilder::ButtonClicked_SideOK()
			{
			...
			void DeckBuilder::ButtonClicked_YES()
			{
				mainGame->HideElement(mainGame->wQuery);
			...
			void DeckBuilder::ScrollBarChanged_CardText()
			{
			...
			void DeckBuilder::EditBoxEnter_KeyWord()
			{
				irr::SEvent me;
			...
			void DeckBuilder::ComboBoxChanged_SortType()
			{
				SortList();
				mainGame->env->setFocus(0);
			}

			void DeckBuilder::LeftMousePressedDown(const irr::SEvent &event)
			{
				...
				is_draging = true;
				mainGame->soundEffectPlayer->doCardPickUp();
			}

			void DeckBuilder::LeftMouseLeftUp()
			{
				if (!is_draging)
					return;
				if (!mainGame->is_siding) {
					...
					if ((hovered_pos == 1 || (hovered_pos == 0 && click_pos == 1)) && deckManager.current_deck.main.size() < 60) {
						...
						mainGame->soundEffectPlayer->doCardAttached();
						is_draging = false;
					}
			...
			void DeckBuilder::RightMouseLeftUp()
			{
				if (mainGame->is_siding) {
					if (is_draging)
						return;
			...
			void DeckBuilder::MiddleMouseLeftUp()
			{
				if (mainGame->is_siding)
					return;
				if (mainGame->wCategories->isVisible() || mainGame->wQuery->isVisible())
			...
			void DeckBuilder::MouseMoved(const irr::SEvent &event)
			{
			...
			void DeckBuilder::MouseWhell(const irr::SEvent &event)
			{
			...
			void DeckBuilder::KeyInput_PresedR(const irr::SEvent &event)
			{
				if (!event.KeyInput.PressedDown && !mainGame->HasFocus(EGUIET_EDIT_BOX))
					mainGame->textFont->setTransparency(true);
			}

			void DeckBuilder::KeyInput_PressedESC()
			{
