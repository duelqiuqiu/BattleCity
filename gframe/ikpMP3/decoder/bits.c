...
unsigned int show_bits__(const GetBitContext *s, int n)
{
    int i;
...
void skip_bits__(GetBitContext *s, int n)
{
    s->index += n;
}

unsigned int get_bits__(GetBitContext *s, int n)
{
    unsigned int result = show_bits__(s, n);
    skip_bits__(s, n);
    return result;
}
