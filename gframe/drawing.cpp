...
namespace ygo {
	...
	void Game::DrawCard(ClientCard* pcard) {
		...
		matManager.mCard.TextureLayer[0].TrilinearFilter = true;
		driver->setTransform(irr::video::ETS_WORLD, pcard->mTransform);
	...
	if(dField.chains.size() > 1) {
		...
		lpcFont->draw(dataManager.GetNumString(dInfo.turn), recti(635 * mainGame->xScale, 5 * mainGame->yScale, 687 * mainGame->xScale, 40 * mainGame->yScale), {255,34,168,172}, true, false, 0); // 改
		ClientCard* pcard;
		for(int i = 0; i < 5; ++i) {
	...
	void Game::DrawSpec() {
		if(showcard) {
			switch(showcard) {
			case 1: {
				EnableMatGuard matGuard;
				auto theImage = imageManager.GetTexture(showcardcode);
				driver->draw2DImage(imageManager.GetTexture(showcardcode), recti(574 * mainGame->xScale, 150 * mainGame->yScale, (574 + 177) * mainGame->xScale, (150 + 254) * mainGame->yScale), recti({ 0, 0 }, theImage->getOriginalSize()), 0, 0, true);
			...
			case 3: {
				EnableMatGuard matGuard;
				auto theImage = imageManager.GetTexture(showcardcode);
				driver->draw2DImage(theImage, recti(574 * mainGame->xScale, 150 * mainGame->yScale, (574 + 177) * mainGame->xScale, (150 + 254) * mainGame->yScale), recti({ 0, 0 }, theImage->getOriginalSize()), 0, 0, true);
			...
			case 4: {
				...
				auto theImage = imageManager.GetTexture(showcardcode);
				driver->draw2DImage(theImage, recti(574 * mainGame->xScale, 154 * mainGame->yScale, 751 * mainGame->xScale, 404 * mainGame->yScale),
					recti({ 0, 0 }, theImage->getOriginalSize()), 0, matManager.c2d, true);
				if(showcarddif < 255)
					showcarddif += 17;
				break;
			}
			case 5: {
				...
				auto theImage = imageManager.GetTexture(showcardcode);
				driver->draw2DImage(theImage, recti((662 - showcarddif * 0.69685f) * mainGame->xScale, (277 - showcarddif) * mainGame->yScale, (662 + showcarddif * 0.69685f) * mainGame->xScale, (277 + showcarddif) * mainGame->yScale),
					recti({ 0, 0 }, theImage->getOriginalSize()), 0, matManager.c2d, true);
				if(showcarddif < 127)
					showcarddif += 9;
				break;
			...
			case 7: {
				...
				auto theImage = imageManager.GetTexture(showcardcode);
				irr::gui::Draw2DImageQuad(driver, imageManager.GetTexture(showcardcode), recti({ 0, 0 }, theImage->getOriginalSize()), corner);
				showcardp++;
				showcarddif += 9;
				if(showcarddif >= 90)
					showcarddif = 90;
	...
	void Game::ShowElement(irr::gui::IGUIElement * win, int autoframe) {
		if (win == wMainMenu)
		{
			User::Instance()->ShowInfoWindow(true);
		}
		FadingUnit fu;
	...
	void Game::HideElement(irr::gui::IGUIElement * win, bool set_action) {
		if (win == wMainMenu)
		{
			User::Instance()->ShowInfoWindow(false);
		}
		FadingUnit fu;
		fu.fadingSize = win->getRelativePosition();
		for(auto fit = fadingList.begin(); fit != fadingList.end(); ++fit)
	...
	void Game::DrawDeckBd() {
		...
		for (size_t i = 0; i < 7 && i + mainGame->scrFilter->getPos() < deckBuilder.results.size(); ++i) {
			...
			if (ptr->second.type & TYPE_MONSTER) {
				...
				textBuffer[len] = form;
				myswprintf_((wchar_t *)&textBuffer[len + 1],ARRAYSIZE(textBuffer) - (len + 1) , L"%d", ptr->second.level);
