...
void CGUIImage::draw()
{
	...
	auto prevMat = driver->getMaterial2D();
	bool prevEnabledMat = driver->getIsEnabledMaterial2D();
	auto& mat = driver->getMaterial2D();
	for (int i = 0; i < 4; i++)
	{
		mat.TextureLayer[i].BilinearFilter = Linear;
		mat.TextureLayer[i].TrilinearFilter = Trilinear;
		mat.TextureLayer[i].AnisotropicFilter = Anisotropic;
	}
	...
	mat = prevMat;
	driver->enableMaterial2D(prevEnabledMat);
...
void CGUIImage::setTextureFilter(bool linear, bool trilinear, int anisotropic)
{
	Linear = linear;
	Trilinear = trilinear;
